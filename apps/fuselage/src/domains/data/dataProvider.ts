...
import { generateFilters, generateSorting, camelizeKeys, upperCaseValues } from './utils'
...
const dataProvider = (client: GraphQLClient, options?: HasuraDataProviderOptions): Required<DataProvider> => {
  ...
  return {
    getOne: async ({ resource, id, meta }) => {
      ...
      const { query, variables } = gql.query({
        operation: defaultNamingConvention ? operation : camelizedOperation,
        variables: {
          id: {
            value: id,
    ...
    getList: async ({ resource, sorters, filters, pagination, meta }) => {
      ...
      const { query, variables } = gql.query([
        ...
        {
          operation: camelizedAggregateOperation,
      ...
      return {
        data: result[operation],
        total: result[camelizedAggregateOperation].aggregate.count,
      }
    },
    ...
    create: async ({ resource, variables, meta }) => {
      ...
      const { query, variables: gqlVariables } = gql.mutation({
        operation: defaultNamingConvention ? insertOperation : camelizedInsertOperation,
        variables: {
          object: {
            type: insertType,
            value: variables,
    ...
    update: async ({ resource, id, variables, meta }) => {
      ...
      const { query, variables: gqlVariables } = gql.mutation({
        operation: defaultNamingConvention ? updateOperation : camelizedUpdateOperation,
        variables: {
    ...
    deleteOne: async ({ resource, id, meta }) => {
      ...
      const { query, variables } = gql.mutation({
        operation: defaultNamingConvention ? deleteOperation : camelizedDeleteOperation,
        variables: {
          id: {
            value: id,
            type: getIdType(resource),
    ...
    getApiUrl: () => {
      throw new Error('getApiUrl method is not implemented on refine-hasura data provider.')
    },
